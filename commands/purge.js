...
module.exports = {
	...
	aliases: ['clear'],
	usage: '[user <mention> <amount to purge] [<amount to purge>]',
    ...
    if (!message.member.hasPermission('MANAGE_MESSAGES')) {
		...
		if (args[0] === 'user') {
			...
			if (!user) return message.reply('You need to specify which user to purge messages from.')
			let amount = args[2]; // Amount of messages which should be deleted
